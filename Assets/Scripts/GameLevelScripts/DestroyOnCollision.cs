...
public class DestroyOnCollision : MonoBehaviour {
	...
	void OnTriggerEnter2D (Collider2D other){
		...
		if (other.gameObject.tag == "enemy") {
			...
			if (Abilites.Explosion.isActive && this.gameObject.name == "Arrow(Clone)") {

				Instantiate (arrowExplosion, other.gameObject.transform.position, other.transform.rotation);
